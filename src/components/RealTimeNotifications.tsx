  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 w-80">
      {notifications.map((notif) => (
        <div 
          key={notif.id} 
          className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-800 p-4 transform transition-all animate-in slide-in-from-bottom-5 fade-in duration-300"
        >
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-1">New Ticket Received!</h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate mb-0.5">{notif.studentEmail}</p>
              <p className="text-sm text-slate-700 dark:text-slate-300 line-clamp-1 mb-3 font-medium">{notif.subject}</p>
              <Link 
            ...
            <button 
              onClick={() => dismissNotification(notif.id)}
              className="w-6 h-6 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors shrink-0"
            >
